...
INSTALL_OPENSSL=true INSTALL_MINIMUM_DEPENDENCIES=true INSTALL_PREFIX=$INSTALL_PREFIX PREREQS_ENV=$PREREQS_ENV scripts/prereqs/install_prereqs.sh
source $PREREQS_ENV
echo "cat $PREREQS_ENV"
cat $PREREQS_ENV
echo "OPENSSL_ROOT_DIR=$OPENSSL_ROOT_DIR"
echo "LD_LIBRARY_PATH=$LD_LIBRART_PATH"
export LD_LIBRARY_PATH=$INSTALL_PREFIX/lib64:$INSTALL_PREFIX/lib:$LD_LIBRARY_PATH
echo "LD_LIBRARY_PATH=$LD_LIBRART_PATH"
mkdir build && cd build
if [[ $(uname) == "Darwin" ]]; then
  cmake -DCMAKE_OSX_ARCHITECTURES="arm64;x86_64" -DCMAKE_PREFIX_PATH=$INSTALL_PREFIX -DBUILD_EXAMPLES=False -DDISABLE_MPI=True -DDISABLE_OPENMP=True -DUSE_HDFS=False ..
else
  export LD_LIBRARY_PATH=$INSTALL_PREFIX/lib64:$INSTALL_PREFIX/lib:$LD_LIBRARY_PATH
  cmake -DBUILD_EXAMPLES=False -DCMAKE_PREFIX_PATH=$INSTALL_PREFIX -DDISABLE_MPI=True -DDISABLE_OPENMP=True -DUSE_HDFS=False ..
fi
make && make install
popd

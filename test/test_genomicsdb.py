...
def test_connect_and_query_with_protobuf(setup):
    ...
    assert (
        len(
            gdb.query_variant_calls(
                query_protobuf=query_config, json_output=json_output_mode.NUM_CALLS
            )
        )
        == 15
    )
    assert (
        len(
            gdb.query_variant_calls(
                query_protobuf=query_config, json_output=json_output_mode.SAMPLES
            )
        )
        == 31
    )
    ...
    assert (
        len(
            gdb.query_variant_calls(
                query_protobuf=query_config, json_output=json_output_mode.ALL_BY_CALLS
            )
        )
        == 207
    )
    assert (
        len(gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.ALL))
        == 273
    )
    with pytest.raises(Exception):
        output_json = gdb.query_variant_calls(query_protobuf=query_config, json_output=9999)
    ...
    print(
        gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.NUM_CALLS)
    )
    assert (
        len(
            gdb.query_variant_calls(
    ...
    output = gdb.query_variant_calls(
        query_protobuf=query_config, json_output=json_output_mode.SAMPLES
    )
    ...
    with pytest.raises(Exception):
        gdb.query_variant_calls(
            query_protobuf=query_config, array="t0_1_2", column_ranges=[], row_ranges=[]
        )

...
def test_connect_and_query_with_protobuf(setup):
    ...
    assert len(gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.NUM_CALLS)) == 15
    assert len(gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.SAMPLES)) == 31
    assert (
        len(gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.SAMPLES_WITH_NUM_CALLS))
        == 37
    )
    assert len(gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.ALL_BY_CALLS)) == 207
    assert len(gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.ALL)) == 273
    with pytest.raises(Exception):
        gdb.query_variant_calls(query_protobuf=query_config, json_output=9999)
    ...
    batch = None
    for output in gdb.query_variant_calls(row_ranges=[(0, 3)], array="t0_1_2", arrow_output=True, batching=True):
        reader = pa.ipc.open_stream(output)
        batch = reader.read_next_batch()
        assert batch.num_columns == 6
    ...
    print(gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.NUM_CALLS))
    assert len(gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.NUM_CALLS)) == 15
    assert len(gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.SAMPLES)) == 2
    assert (
        len(gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.SAMPLES_WITH_NUM_CALLS))
        == 2
    )
    assert len(gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.ALL_BY_CALLS)) == 2
    assert len(gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.ALL)) == 2
    ...
    output = gdb.query_variant_calls(query_protobuf=query_config, json_output=json_output_mode.SAMPLES)
